...
### 技术一面 0618
半小时聊天，问了问实习，手撕一道easy，求dfs岛屿数量

### 技术二面 0619
实习、项目、问完两道手撕，一道纯英文的判断出栈序列合法，一道数组中数字组成的小于n的最大数，写了一个小时，第二题还没A完，汗流浃背
...
在这个项目中，我参考了MIT的操作系统课程（6.S081），并在 RISCV架构 上实现了一个精简版操作系统的核心功能。项目的目的是帮助深入理解操作系统的设计与实现，重点包括以下几个方面：

操作系统核心功能实现：

进程管理： 实现了基本的进程调度和进程创建/终止，采用时间片轮转算法来管理CPU时间分配。

内存管理： 实现了虚拟内存和页表管理，优化内存的使用和管理。

系统调用： 实现了用户程序和操作系统之间的交互接口，支持文件操作、内存分配等基本功能。

并发与同步：

实现了 互斥锁、信号量 和 条件变量，确保多个进程/线程之间的同步，避免竞态条件和死锁。

使用 银行家算法 来避免死锁并保证系统资源的有效分配。

文件系统与设备管理：

实现了简化版的 文件系统，支持文件的创建、读取和删除。

编写了 设备驱动程序，实现操作系统与硬件（如磁盘、输入设备等）的交互。
#### 困难点 -->
